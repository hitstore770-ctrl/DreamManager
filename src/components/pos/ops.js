...
// Blind stocktake
//
// The whole point is that the expected quantity is *hidden* while counting.
// Showing it first is how a count turns into a rubber stamp: you see 12, you
// see roughly twelve things, you type 12. So the system number only appears
// after a figure is committed — and the variance it reveals is the actual
// output of the exercise.

export function BlindCount() {
  const { inventory, setInventory } = useBusiness();
  const [counts, setCounts] = usePersistentState("@dreammanager/pos-blind-count", {});
  const [draft, setDraft] = useState("");
  const [idx, setIdx] = useState(0);

  const items = inventory || [];
  const pending = items.filter((i) => counts[i.id] == null);
  const current = pending[Math.min(idx, Math.max(0, pending.length - 1))] || null;

  const counted = Object.keys(counts).filter((id) => items.some((i) => i.id === id));
  const variances = counted
    .map((id) => {
      const item = items.find((i) => i.id === id);
      return { id, name: item.name, expected: item.qty, actual: counts[id], diff: counts[id] - item.qty };
    })
    .filter((v) => v.diff !== 0);
  ...
  const commit = () => {
    ...
    hapticLight();
    setCounts((prev) => ({ ...prev, [current.id]: n }));
    setDraft("");
    setIdx(0);
  };

  // Applying the count is the destructive half, so it is a separate, explicit
  // action — never a side effect of typing a number.
  const applyAll = () => {
    if (!counted.length) return;
    hapticSuccess();
    setInventory((prev) => prev.map((i) => (counts[i.id] != null ? { ...i, qty: counts[i.id] } : i)));
    setCounts({});
  };

  if (!items.length) {
    return <Empty icon="package" title="אין פריטים במלאי" sub="הוסף פריטים במחסן, ואז אפשר לספור אותם כאן." />;
  }

  return (
    <View style={st.wrap}>
      <CustomText style={kit.hint}>
        הכמות שבמערכת מוסתרת בזמן הספירה בכוונה. היא תיחשף רק אחרי שתזין מספר — אחרת הספירה רק מאשרת את מה שכבר רשום.
      </CustomText>

      {current ? (
        <View style={st.blindCard}>
          <CustomText style={st.blindName}>{current.name}</CustomText>
          <CustomText style={st.blindSub}>
            נספרו {counted.length} מתוך {items.length}
          </CustomText>
          <TextInput
            testID="blind-input"
            style={st.blindInput}
            ...
            placeholder="כמה יש בפועל?"
            placeholderTextColor={INK_MUTED}
            textAlign="center"
            onSubmitEditing={commit}
          />
          <View style={kit.row}>
            <TouchableOpacity testID="blind-commit" style={[kit.actionBtn, { backgroundColor: BLUE }]} onPress={commit}>
              <CustomText style={kit.actionText}>אישור וספירה הבאה</CustomText>
            </TouchableOpacity>
            {pending.length > 1 && (
              <TouchableOpacity
                style={[kit.actionBtn, { flex: 0, paddingHorizontal: 18, backgroundColor: CARD }]}
                onPress={() => {
                  hapticLight();
                  setDraft("");
              ...
              >
                <CustomText style={[kit.actionText, { color: INK_SOFT }]}>דלג</CustomText>
              </TouchableOpacity>
            )}
          </View>
        </View>
      ) : (
        <View style={[st.banner, { backgroundColor: GREEN + "12" }]}>
          <CustomText style={[kit.bannerText, { color: GREEN }]}>הכל נספר</CustomText>
          <CustomText style={[kit.bannerSub, { color: INK_SOFT }]}>אפשר להחיל את הספירה על המלאי, או לאפס ולהתחיל מחדש.</CustomText>
        </View>
      )}

      {!!counted.length && (
        <>
          <CustomText style={kit.sectionLabel}>הפרשים</CustomText>
          {variances.length === 0 ? (
            <CustomText style={kit.hint}>אין הפרשים בין הספירה למערכת עד כה.</CustomText>
          ) : (
            variances.map((v) => (
              <View key={v.id} style={st.varRow}>
                <CustomText style={[st.varDiff, { color: v.diff < 0 ? RED : GREEN }]}>
                  {v.diff > 0 ? "+" : ""}
                  {v.diff}
                </CustomText>
                <View style={{ flex: 1 }}>
                  <CustomText style={st.varName}>{v.name}</CustomText>
                  <CustomText style={st.varSub}>
                    במערכת {v.expected} · נספר {v.actual}
                  </CustomText>
                </View>
              </View>
            ))
          ...
          <View style={kit.row}>
            <TouchableOpacity testID="blind-apply" style={[kit.actionBtn, { backgroundColor: GREEN }]} onPress={applyAll}>
              <CustomText style={kit.actionText}>עדכן מלאי לפי הספירה</CustomText>
            </TouchableOpacity>
            <TouchableOpacity
              style={[kit.actionBtn, { flex: 0, paddingHorizontal: 18, backgroundColor: CARD }]}
              onPress={() => {
            ...
            >
              <CustomText style={[kit.actionText, { color: INK_SOFT }]}>אפס</CustomText>
            </TouchableOpacity>
          </View>
        </>
      )}
...
// ---------------------------------------------------------------------------
// Stock forecast — days of cover left, from the real sales history.
//
// Deliberately reports "אין מספיק נתונים" rather than a number when an item
// has no sales in the window. A forecast built on a zero-sales denominator is
// either infinity or a divide by zero, and neither is a useful thing to show
// on a shelf label.

export function StockForecast() {
  const { inventory, sales } = useBusiness();
  const [days, setDays] = useState(14);

  const rows = useMemo(() => {
    const since = Date.now() - days * 86400000;
    const window = (sales || []).filter((r) => r.ts >= since && r.kind === "sale");
    return (inventory || [])
      .map((item) => {
        const sold = window.filter((r) => r.itemId === item.id).reduce((n, r) => n + (r.qty || 0), 0);
        const perDay = sold / days;
        const cover = perDay > 0 ? item.qty / perDay : null;
        return { ...item, sold, perDay, cover };
      })
      .sort((a, b) => {
  ...
  if (!rows.length) {
    return <Empty icon="package" title="אין פריטים במלאי" sub="התחזית נבנית מהמלאי ומהמכירות בפועל." />;
  }

  return (
    <View style={st.wrap}>
      <CustomText style={kit.sectionLabel}>חלון חישוב</CustomText>
      <View style={kit.segment}>
        {[7, 14, 30].map((d) => (
          <TouchableOpacity
            key={d}
            style={[kit.segmentBtn, days === d && kit.segmentOn]}
            onPress={() => {
              hapticLight();
              setDays(d);
            }}
          >
            <CustomText style={[kit.segmentText, days === d && { color: WHITE }]}>{d} ימים</CustomText>
          </TouchableOpacity>
        ))}
      </View>

      {rows.map((r) => (
        <View key={r.id} style={st.varRow}>
          <CustomText style={[st.varDiff, { color: r.cover == null ? INK_MUTED : r.cover < 3 ? RED : r.cover < 7 ? AMBER : GREEN, fontSize: 15 }]}>
            {r.cover == null ? "—" : `${Math.floor(r.cover)} ימים`}
          </CustomText>
          <View style={{ flex: 1 }}>
            <CustomText style={st.varName}>{r.name}</CustomText>
            <CustomText style={st.varSub}>
              {r.cover == null
                ? "אין מספיק נתוני מכירה בחלון הזה"
                : `במלאי ${r.qty} · ${r.perDay.toFixed(1)} ליום`}
            </CustomText>
          </View>
        </View>
      ))}
      <CustomText style={kit.hint}>
        פריט בלי מכירות בחלון שנבחר לא מקבל תחזית. קצב אפס היה מייצר "אינסוף ימים", וזה מספר שנראה בטוח בדיוק כשהוא חסר משמעות.
      </CustomText>
    </View>
  );
}
